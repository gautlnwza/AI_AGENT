...
**my_project** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg), JWT + API Key auth, Redis, PydanticAI, Next.js 15 (i18n)
## Commands
...
│   ├── user.py           #   thin: just a class with db + repo calls├── agents/               # AI agent wrappers + tools
...
- `@field_validator` for deserialization (e.g., JSON string → dict for SQLite)- IDs are `UUID` type
## Exception Handling
