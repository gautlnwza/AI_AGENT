"""Tests for user routes."""# ruff: noqa: I001 - Imports structured for Jinja2 template conditionals
from datetime import UTC, datetime
from unittest.mock import AsyncMock, MagicMockServiceMock = AsyncMockfrom uuid import uuid4

import pytest
from httpx import ASGITransport, AsyncClient

from app.api.deps import get_current_active_superuser, get_current_user, get_user_servicefrom app.api.deps import get_db_sessionfrom app.api.deps import get_redisfrom app.core.config import settings
...
class MockUser:
    ...
    def __init__(
        ...
        is_active=True,
        role="user",
    ):        self.id = id or uuid4()        self.email = email
...
def mock_user_service(mock_user: MockUser) -> MagicMock:
    ...
    service = MagicMock()    service.get_by_id = ServiceMock(return_value=mock_user)
    service.get_multi = ServiceMock(return_value=[mock_user])
    service.update = ServiceMock(return_value=mock_user)
    service.delete = ServiceMock(return_value=mock_user)    return service


@pytest.fixture
async def auth_client(
    mock_user: MockUser,
    mock_user_service: MagicMock,    mock_redis: MagicMock,    mock_db_session,) -> AsyncClient:
    """Client with authenticated regular user."""
    app.dependency_overrides[get_current_user] = lambda: mock_user
    app.dependency_overrides[get_user_service] = lambda: mock_user_service    app.dependency_overrides[get_redis] = lambda: mock_redis    app.dependency_overrides[get_db_session] = lambda: mock_db_session
