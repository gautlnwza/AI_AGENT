...
class MockConversation:
    ...
    def __init__(
        self,
        id=None,
        title="Test Conversation",        user_id=None,        is_archived=False,
    ):
        self.id = id or uuid4()
        self.title = title        self.user_id = user_id        self.is_archived = is_archived
...
class TestConversationServiceCreate:
    ...
    async def test_create_conversation(self, service: ConversationService):
        ...
        mock_data = MagicMock()        mock_data.user_id = uuid4()        mock_data.title = "Test Conversation"
...
class TestConversationServiceExportAll:
    ...
    async def test_export_all_with_conversations(self, service: ConversationService):
        ...
        with patch("app.services.conversation.conversation_repo") as mock_repo:
            ...
            mock_repo.count_messages = AsyncMock(return_value=1)            # Patch the *use site* — service does `from app.repositories import
