...
async def list_slash_commands(service: UserSlashCommandSvc, user: CurrentUser) -> Any:
    """List the current user's custom commands and built-in overrides."""    items, total = await service.list_for_user(user_id=user.id)    return UserSlashCommandList(
...
async def create_custom_command(
    ...
    """Create a user-defined command with a stored prompt body."""    db_cmd = await service.create_custom(user_id=user.id, data=data)    return UserSlashCommandRead.model_validate(db_cmd)
...
async def upsert_builtin_override(
    ...
    """Toggle a built-in command on or off for the current user."""    db_cmd = await service.upsert_builtin_override(
...
async def update_slash_command(    command_id: UUID,    data: UserSlashCommandUpdate,
    service: UserSlashCommandSvc,
    user: CurrentUser,
) -> Any:
    """Patch a custom command. Built-in overrides accept only ``is_enabled``."""    db_cmd = await service.update(user_id=user.id, command_id=command_id, data=data)    return UserSlashCommandRead.model_validate(db_cmd)


@router.delete(
    "/{command_id}", status_code=status.HTTP_204_NO_CONTENT, response_model=None
)
async def delete_slash_command(    command_id: UUID,    service: UserSlashCommandSvc,
    user: CurrentUser,
) -> Any:
    """Delete a custom command, or remove a built-in override (re-enables it)."""    await service.delete(user_id=user.id, command_id=command_id)    return None