...
from typing import Annotated, Anyfrom uuid import UUID
...
@router.get("", response_model=ItemList)async def list_items(    service: ItemSvc,
    user: CurrentUser,
    skip: int = Query(0, ge=0, description="Items to skip"),
    limit: int = Query(50, ge=1, le=100, description="Max items to return"),
) -> Any:
    """List items owned by the current user."""    items, total = await service.list(owner_id=user.id, skip=skip, limit=limit)    return ItemList(items=items, total=total)


@router.post("", response_model=ItemRead, status_code=status.HTTP_201_CREATED)async def create_item(    data: ItemCreate,
    service: ItemSvc,
    user: CurrentUser,
) -> Any:
    """Create a new item owned by the current user."""    return await service.create(owner_id=user.id, data=data)

@router.get("/{item_id}", response_model=ItemRead)async def get_item(
    item_id: UUID,
    service: ItemSvc,
    user: CurrentUser,
) -> Any:
...
@router.patch("/{item_id}", response_model=ItemRead)async def update_item(
    item_id: UUID,
    data: ItemUpdate,
    service: ItemSvc,
    user: CurrentUser,
