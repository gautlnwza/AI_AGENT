...
Reference for `my_project` runtime configuration. The
...
|---|---|---|---|
| `PROJECT_NAME` | optional | `my_project` | Used in logs, OpenAPI title, email templates |
...
| `TIMEZONE` | optional | `` | IANA TZ name (e.g. `Europe/Warsaw`) |
...
| `REFRESH_TOKEN_EXPIRE_MINUTES` | optional | `10080` | JWT refresh token lifetime (7 days) || `GOOGLE_OAUTH_CLIENT_ID` | required | — | From Google Cloud Console → OAuth credentials |
...
## LLM / AI
| `GOOGLE_API_KEY` | **required** | — | From aistudio.google.com || `LOGFIRE_TOKEN` | optional | — | When set, ships traces to Logfire (logfire.pydantic.dev) |
## Redis
