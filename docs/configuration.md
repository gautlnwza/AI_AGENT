...
|----------|---------|-------------|
| `PROJECT_NAME` | `my_project` | Display name for the project |
...
| `TIMEZONE` | `` | IANA timezone (e.g. `UTC`, `Europe/Warsaw`, `America/New_York`) |
...
| `POSTGRES_DB` | `my_project` | Database name |
...
| `AI_MODEL` | `gemini-2.5-flash` | Default LLM model for chat || `AI_TEMPERATURE` | `0.7` | LLM temperature (0.0 = deterministic, 1.0 = creative) |
...
| `LOGFIRE_SERVICE_NAME` | `my_project` | Service name in Logfire dashboard |
...
| `ACME_EMAIL` | `admin@example.com` | Let's Encrypt email for SSL certs || `REDIS_PASSWORD` | `change-me-in-production` | Redis password for production |
## Production Checklist

Before deploying to production, ensure these variables are properly set:1. `SECRET_KEY` -- Generate a unique 64-character hex key: `openssl rand -hex 32`3. `ENVIRONMENT` -- Set to `production`
4. `DEBUG` -- Set to `false`5. `POSTGRES_PASSWORD` -- Use a strong, unique password6. `CORS_ORIGINS` -- List only your actual frontend domain(s)7. `REDIS_PASSWORD` -- Set a strong password8. `GOOGLE_API_KEY` -- Your production API key