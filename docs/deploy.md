...
- ✅ Docker / `docker-compose.yml`- ❌ No Kubernetes manifests- CI: `github`
...
### Reverse proxyTraefik labels in `docker-compose.yml` route based on `Host()`. Set `DOMAIN` env var, then point your DNS at the host. ACME / Let's Encrypt configured via labels — uncomment in `docker-compose.yml` and set `ACME_EMAIL`.
...
```bash
fly launch --name my_project-backend --region waw
fly postgres create --name my_project-db
fly postgres attach my_project-db
# Redis: use Upstash (`fly redis create`) or Fly's Tigrisfly secrets set $(cat backend/.env | grep -v '^#' | xargs)
fly deploy
```
...
- [ ] Logs flowing to your aggregator + Logfire receiving traces- [ ] Reverse proxy enforces HTTPS
