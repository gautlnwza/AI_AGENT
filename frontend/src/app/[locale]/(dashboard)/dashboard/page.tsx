...
import {  ArrowUpRight,  Database,  MessageSquare,  Search,} from "lucide-react";import { OnboardingBanner } from "@/components/dashboard/onboarding-banner";
import { QuickActions } from "@/components/dashboard/quick-actions";
import { RecentActivity } from "@/components/dashboard/recent-activity";import { StatCard } from "@/components/dashboard/stat-card";import { useAuth } from "@/hooks";
import { apiClient } from "@/lib/api-client";
import { ROUTES } from "@/lib/constants";
import { cn } from "@/lib/utils";import type { HealthResponse } from "@/types";
...
export default function DashboardPage() {
  ...
  const [healthError, setHealthError] = useState(false);  const [conversations, setConversations] = useState<{ total: number } | null>(null);
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .finally(() => setConvLoading(false));    setRagStats({ collections: 0, vectors: 0 });  }, []);
...
<h1 className="font-display text-foreground mt-2 text-3xl leading-[1.05] font-bold tracking-tight sm:text-4xl [&_em]:font-accent [&_em]:font-normal [&_em]:italic">
        ...
        <div className="border-foreground/10 bg-foreground/[0.02] relative flex flex-col justify-between gap-6 overflow-hidden rounded-3xl border p-6 sm:p-7">
          ...
          <dl className="space-y-2.5 text-xs">          </dl>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-2">        <StatCard
          label="Conversations"
