...
import { ThemeToggle } from "@/components/theme";import { APP_NAME, ROUTES } from "@/lib/constants";
import { cn } from "@/lib/utils";
import {
  LogOut,
  Menu,
  LayoutDashboard,
  MessageSquare,  UserCircle,  ShieldCheck,
} from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui";
import { useSidebarStore } from "@/stores";
const adminNavItems = [
  { name: "Dashboard", href: ROUTES.DASHBOARD, icon: LayoutDashboard, adminOnly: false },
  { name: "Chat", href: ROUTES.CHAT, icon: MessageSquare, adminOnly: false },  { name: "Profile", href: ROUTES.PROFILE, icon: UserCircle, adminOnly: false },
  ...
  return (
    <header className="bg-background/95 supports-[backdrop-filter]:bg-background/60 sticky top-0 z-40 w-full border-b backdrop-blur">
      <div className="flex h-14 items-center justify-between px-3 sm:px-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">          <ThemeToggle />
          {isAuthenticated ? (
            <>
