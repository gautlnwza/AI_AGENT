...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app my_project db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		my_project user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app my_project db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app my_project db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run my_project db migrate -m "initial" || true
	cd backend && uv run my_project db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend my_project db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend my_project db upgrade

db-downgrade:
	uv run --directory backend my_project db downgrade

db-current:
	uv run --directory backend my_project db current

db-history:
	uv run --directory backend my_project db history

# === Server ===
run:
	uv run --directory backend my_project server run --reload

run-prod:
	uv run --directory backend my_project server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend my_project server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend my_project user create-admin

user-create:
	uv run --directory backend my_project user create

user-list:
	uv run --directory backend my_project user list
...
help:
	@echo ""
	@echo "my_project - Available Commands"
	@echo "======================================"
	@echo ""
